...
def filter_dotplot_data(proportion_matrix, expression_matrix, rows1, rows2, meta_data,
    ...
    return filtered_curation, filtered_prop_matrix, filtered_expr_matrix, filtered_rows1, filtered_rows2


def create_dotplot(proportion_matrix, expression_matrix, genes1, genes2, rows1, rows2, selected_genes, selected_cell_types=None):
    """
    ...
    try:
        # Data processing 
        proportion_matrix = proportion_matrix.toarray() if hasattr(proportion_matrix, 'toarray') else np.array(proportion_matrix)
        expression_matrix = expression_matrix.toarray() if hasattr(expression_matrix, 'toarray') else np.array(expression_matrix)
        ...
        for gene in selected_genes:
            ...
            grouped = temp_df.groupby('cell_type').agg({
                'proportion': 'mean',
                'expression': 'mean',
        ...
        fig1.add_trace(go.Scatter(
            ...
            customdata=plot_df[['Proportion', 'Mean_Expression', 'Number_of_datasets']].values,
            name=''
        ))
        # Create legend plot
        ...
        fig2.update_layout(
            ...
            margin=dict(l=10, r=10, t=10, b=10),  # Adjust margins to accommodate larger text
...
def create_ligand_receptor_plot(liana_df, selected_source=None, selected_target=None, top_n=50, sort_by='magnitude', order_by='sender'):
    """
    ...
    liana_df['interaction'] = liana_df['ligand_complex'] + ' → ' + liana_df['receptor_complex']
    ...
    for interaction in top_interactions:
        ...
        scaled_sizes = ((interaction_data['magnitude_rank'] - magnitude_rank_min) / 
                        (magnitude_rank_max - magnitude_rank_min)) * DOT_SIZE
    ...
    for pair in cell_pairs:
        ...
        if not hover_data[pair]:
            fig.add_trace(go.Scatter(
                x=[pair],
                y=['No Data'] if 'No Data' not in fig.data else [f'No Data {len(fig.data)}'],
                mode='markers',
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                colorscale=[[0, 'lightgrey'], [1, 'red']],
                showscale=pair == cell_pairs[0],  # Only show colorbar for first trace
                colorbar=dict(
                    title=dict(
            ...
            customdata=[[data['specificity_rank'], data['magnitude_rank']] for data in pair_data],
            showlegend=False
        ))
    
    # Create magnitude legend with values spanning the full range
    num_legend_points = 6
    legend_magnitude_ranks = np.linspace(magnitude_rank_min, magnitude_rank_max, num_legend_points)
    ...
    legend_sizes = ((legend_magnitude_ranks - magnitude_rank_min) / 
                    (magnitude_rank_max - magnitude_rank_min)) * DOT_SIZE
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            constrain='domain',
            tickmode='array',
            ticktext=top_interactions + (['No Data'] if any(not data for data in hover_data.values()) else []),
            tickvals=list(range(len(top_interactions) + (1 if any(not data for data in hover_data.values()) else 0))),
            range=[-0.5, len(top_interactions) + 0.5]
        ),
        xaxis2=dict(
            domain=[0.9, 0.95],  # Dot size legend takes 0.80-0.90
            visible=False
        ),
        yaxis2=dict(
            anchor='x2',
        ...
        coloraxis_colorbar=dict(
            x=1,  # Slightly adjusted to fit within 0.90-1.0
            len=1
        ),
        plot_bgcolor='white'
    )
