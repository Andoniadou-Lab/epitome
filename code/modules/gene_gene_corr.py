...
def create_gene_correlation_plot(gene1_name, gene2_name, base_path, meta_data, selected_samples=None, 
                               color_by_celltype=True, selected_cell_types=None):
    """
    ...
    try:
        ...
        if color_by_celltype:
            fig = px.scatter(plot_df,
                           ...
                           labels={'Gene1': f'{gene1_name} Expression',
        ...
        cell_type_stats = plot_df.groupby('Cell_Type').agg({
            'Gene1': ['count', lambda x: x.corr(plot_df.loc[x.index, 'Gene2'])]
        }).round(3)
