...
def load_and_transform_data(version="v_0.01"):
    ...
    """
    matrix = scipy.io.mmread(f'{BASE_PATH}/data/expression/{version}/normalized_data.mtx')
    genes = pd.read_parquet(f'{BASE_PATH}/data/expression/{version}/genes.parquet')
    if len(genes.columns) == 1:
        genes.columns = [0]
    meta_data = pd.read_parquet(f'{BASE_PATH}/data/expression/{version}/meta_data.parquet')
    meta_data = meta_data[["new_cell_type", "sample", "Age_numeric", "sc_sn_atac", "Comp_sex","Name","Author","Normal","SRA_ID","Sorted"]]
    
    # Print age range for debugging
    print(f"Age range in data: {meta_data['Age_numeric'].min()} to {meta_data['Age_numeric'].max()}")
...
def load_annotation_data(version="v_0.01"):
    ...
    """
    return pd.read_parquet(f'{BASE_PATH}/data/accessibility/{version}/annotation.parquet')
...
def load_motif_data(version="v_0.01", just_motif_names=False):
    ...
    data = pl.read_parquet(f'{BASE_PATH}/data/accessibility/{version}/atac_motif_data.parquet')
...
def load_chromvar_data(version="v_0.01"):
    ...
    Load ChromVAR data
    """
    chromvar_matrix = scipy.io.mmread(f'{BASE_PATH}/data/chromvar/{version}/normalized_data.mtx')
    chromvar_meta = pd.read_parquet(f'{BASE_PATH}/data/chromvar/{version}/meta_data.parquet')
    
    # Read features and columns from parquet, but process them like text files
    features_df = pd.read_parquet(f'{BASE_PATH}/data/chromvar/{version}/chromvar_features.parquet')
    features = features_df[features_df.columns[0]].tolist()
    
    columns_df = pd.read_parquet(f'{BASE_PATH}/data/chromvar/{version}/chromvar_columns.parquet')
...
def load_isoform_data(version="v_0.01"):
    ...
    """
    matrix = scipy.sparse.load_npz(f"{BASE_PATH}/data/isoforms/{version}/isoforms_matrix.mtx.npz")
    
    # Read with no header behavior
    features = pd.read_parquet(f"{BASE_PATH}/data/isoforms/{version}/isoforms_features.parquet")
    samples = pd.read_parquet(f"{BASE_PATH}/data/isoforms/{version}/isoforms_samples.parquet")
    ...
    features[['transcript_id', 'gene_name']] = features[0].str.split('_', n=1, expand=True)
    
    for i in range(len(samples)):
        if len(samples.iloc[i, 0].split('_')) > 2:
            samples.iloc[i, 0] = samples.iloc[i, 0].split('_')[0] + '_' + samples.iloc[i, 0].split('_')[2]
...
def load_dotplot_data(version="v_0.01"):
    ...
    Load dot plot data
    """
    proportion_matrix = scipy.io.mmread(f'{BASE_PATH}/data/dotplot/{version}/matrix2.mtx')
    expression_matrix = scipy.io.mmread(f'{BASE_PATH}/data/dotplot/{version}/matrix1.mtx')
    
    # Read all with no header behavior
    genes1 = pd.read_parquet(f'{BASE_PATH}/data/dotplot/{version}/matrix1_genes.parquet')
    genes2 = pd.read_parquet(f'{BASE_PATH}/data/dotplot/{version}/matrix2_genes.parquet')
...
def load_accessibility_data(version="v_0.01"):
    ...
    """
    accessibility_matrix = scipy.io.mmread(f'{BASE_PATH}/data/accessibility/{version}/normalized_data.mtx')
    accessibility_meta = pd.read_parquet(f'{BASE_PATH}/data/accessibility/{version}/atac_meta_data.parquet')
    
    # Read features and columns from parquet but process them like text files
    features_df = pd.read_parquet(f'{BASE_PATH}/data/accessibility/{version}/accessibility_features.parquet')
    features = features_df[features_df.columns[0]].tolist()
    
    columns_df = pd.read_parquet(f'{BASE_PATH}/data/accessibility/{version}/accessibility_columns.parquet')
    columns = columns_df[columns_df.columns[0]].tolist()
...
def load_marker_data(version="v_0.01"):
    ...
    """
    cell_typing_markers = pd.read_parquet(f'{BASE_PATH}/data/markers/{version}/cell_typing_markers.parquet')
    grouping_lineage_markers = pd.read_parquet(f'{BASE_PATH}/data/markers/{version}/grouping_lineage_markers.parquet')
    ...
    cell_typing_markers = cell_typing_markers.merge(cpdb,how="left",left_on="gene",right_on="gene")
    grouping_lineage_markers = grouping_lineage_markers.merge(cpdb,how="left",left_on="gene",right_on="gene")

    #from grouping_lineage markers, remove cols log2fc pvalue mean_AvgExpr TF
    grouping_lineage_markers = grouping_lineage_markers.drop(columns=["log2fc","pvalue","mean_AvgExpr","TF"])
...
def load_proportion_data(version="v_0.01"):
    ...
    """
    abundance_matrix = scipy.io.mmread(f'{BASE_PATH}/data/cell_proportion/{version}/abundance.mtx')
    abundance_rows = pd.read_csv(f'{BASE_PATH}/data/cell_proportion/{version}/abundance_rows.tsv', sep='\t', header=None)
    abundance_cols = pd.read_csv(f'{BASE_PATH}/data/cell_proportion/{version}/abundance_cols.tsv', sep='\t', header=None)
...
def load_atac_proportion_data(version="v_0.01"):
    ...
    """
    try:
        abundance_matrix = scipy.io.mmread(f'{BASE_PATH}/data/cell_proportion_atac/{version}/abundance.mtx')
        abundance_rows = pd.read_csv(f'{BASE_PATH}/data/cell_proportion_atac/{version}/abundance_rows.tsv', sep='\t', header=None)
        abundance_cols = pd.read_csv(f'{BASE_PATH}/data/cell_proportion_atac/{version}/abundance_cols.tsv', sep='\t', header=None)
...
def load_single_cell_dataset(sra_id, version="v_0.01"):
    ...
    """
    import anndata
    return anndata.read_h5ad(f'{BASE_PATH}/sc_data/datasets/{version}/epitome_h5_files/{sra_id}_processed.h5ad', backed='r')
...
def load_aging_genes(version="v_0.01"):
    ...
    # Clean column names
    aging_genes_df.columns = [col.replace('_', ' ').title() for col in aging_genes_df.columns]
...
def load_ligand_receptor_data(version="v_0.01"):
    ...
    liana_df = pd.read_parquet(f'{BASE_PATH}/data/lig_rec/{version}/liana_consensus.parquet')
    ...
    liana_df["interaction"] = liana_df.apply(
        lambda row: tuple(sorted([row["ligand_complex"], row["receptor_complex"],row["source"],row["target"]])), axis=1
    )
    #sort by ligand_complex
    liana_df = liana_df.sort_values(["ligand_complex","corrected_score_y"],ascending=[True,True])
    ...
    liana_df = liana_df.rename(columns={
...
def load_heatmap_data(version="v_0.01"):
    motif_analysis_summary = pd.read_csv(f'{BASE_PATH}/data/heatmap/{version}/all_motif_results.csv')

    #turn each motif.name into capitalized (first letter)
    motif_analysis_summary['motif.name'] = motif_analysis_summary['motif.name'].str.capitalize()
    #if it has . or :, remove it
    motif_analysis_summary['motif.name'] = motif_analysis_summary['motif.name'].str.replace('.', '')
    motif_analysis_summary['motif.name'] = motif_analysis_summary['motif.name'].str.replace(':', '')
    #for each motif, analysis pair, keep the one with the highest fold.enrichment
    motif_analysis_summary = motif_analysis_summary.sort_values('fold.enrichment', ascending=False).drop_duplicates(['motif.name', 'analysis'])


    coefs = pd.read_csv(f'{BASE_PATH}/data/heatmap/{version}/coef.csv', index_col=0)
    rna_res = pd.read_csv(f'{BASE_PATH}/data/heatmap/{version}/rna_grouping_lineage_markers.csv')
    atac_res = pd.read_csv(f'{BASE_PATH}/data/heatmap/{version}/atac_grouping_lineage_markers.csv')
    mat = scipy.io.mmread(f'{BASE_PATH}/data/heatmap/{version}/motif_matrix.mtx')
    features = pd.read_table(f'{BASE_PATH}/data/heatmap/{version}/motif_matrix_rows.txt', header=None)
    columns = pd.read_table(f'{BASE_PATH}/data/heatmap/{version}/motif_matrix_cols.txt', header=None)
    ...
    motif_analysis_summary = motif_analysis_summary[motif_analysis_summary['motif.name'].isin(genes_to_keep)]
    #rename motif.name to gene
    motif_analysis_summary = motif_analysis_summary.rename(columns={'motif.name':'gene'})
