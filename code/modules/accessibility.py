...
def create_accessibility_plot(matrix, features, meta_data, feature_name, additional_group=None, connect_dots=False):
    """
    ...
    if additional_group:
        ...
        for cell_type in cell_types:
            ...
            cell_type_positions = [pos for (ct, _), pos in position_map.items() if ct == cell_type]
    ...
    if connect_dots:
        for sra_id in plot_df['SRA_ID'].unique():
            ...
            x_values = sra_df['x_position'] if 'x_position' in sra_df.columns else sra_df['cell_type']
...
def create_genome_browser_plot(
    ...
    for gene_data in genes_data:
        ...
        for track_idx, track in enumerate(tracks):
            ...
            for existing_gene in track:
                if not (gene_data['end'] < existing_gene['start'] - (region_end - region_start) * 0.05 or 
                       gene_data['start'] > existing_gene['end'] + (region_end - region_start) * 0.05):
                    can_place = False
                    break
    ...
    for i, feature in enumerate(features):
        try:
            ...
            if (feat_chr == chr_name and 
                feat_start <= region_end and 
                feat_end >= region_start):
                feature_data.append({
                    'index': i,
                    'chr': feat_chr,
                    'start': feat_start,
            ...
            for _, feature in feature_df.iterrows():
                try:
                    ...
                    if hasattr(matrix, 'toarray'):
                        ...
                        signal = cell_type_values.toarray().mean() if cell_type_values.size > 0 else 0
                    else:
                        # For dense matrices
                        cell_type_values = matrix[feature['index'], cell_mask]
                        signal = cell_type_values.mean() if cell_type_values.size > 0 else 0
                    ...
                    print(f"Error processing feature {feature['index']} for cell type {cell_type}: {str(e)}")
    ...
    for track_idx, track in enumerate(tracks):
        ...
        for gene_data in track:
            ...
            gene_exons = region_annotations[region_annotations['gene_name'] == gene_name]
    ...
    for i, cell_type in enumerate(cell_types):
        ...
        # Add scale bar
        fig.add_trace(go.Scatter(
            x=[region_start - scale_bar_width * 2, region_start - scale_bar_width * 2],
        ...
        for _, peak in track_data.iterrows():
            normalized_height = (peak['signal'] / max_signal) * accessibility_track_height
    ...
    if selected_motifs:
        ...
        motif_colors = {motif: motif_color_palette[i % len(motif_color_palette)] 
        ...
        for i, motif in enumerate(selected_motifs):
            ...
            for _, site in motif_data.iterrows():
                if site['start'] <= region_end and site['end'] >= region_start:
                    ...
                    fig.add_trace(go.Scatter(
                        x=[motif_start, motif_end, motif_end, motif_start, motif_start],
                        y=[motif_y, motif_y, motif_y + motif_height, motif_y + motif_height, motif_y],
                        mode='none',
                        fill='toself',
    ...
    plot_height = 100 + (len(cell_types) * 80) + (len(tracks) * 60) + motif_height_addition
    y_range_max = ((len(cell_types) + len(tracks)) * track_spacing + 
