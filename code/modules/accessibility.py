...
def create_accessibility_plot(
    ...
    """Box + strip plot for chromatin accessibility with 5th/95th percentile whiskers."""
    feature_idx = features.index(feature_name)
    plot_df = meta_data.copy()
    plot_df["Accessibility"] = to_array(matrix[feature_idx, :])
    ...
    if additional_group:
        ...
        title = (
            f"{feature_name} Accessibility by Cell Type and Sex"
            if additional_group == "Comp_sex"
            else f"{feature_name} Accessibility by Cell Type and {additional_group}"
        )
    ...
    if connect_dots:
        add_connected_sample_lines(fig, plot_df, x_col, "Accessibility")
