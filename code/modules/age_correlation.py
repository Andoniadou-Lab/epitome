...
import numpy as np
def create_age_correlation_plot(matrix, genes, meta_data, gene_name, cell_type, use_log_age=False, remove_zeros=False, 
                              color_by=None, show_trendline=True, data_type_filter=None, version="v_0.01"):
    """
    ...
    fig.update_traces(
        marker=dict(size=15),  # Increased marker size
    ...
    if show_trendline:
        for trace in fig.data:
            if trace.mode == 'lines':
                ...
                if x_intercept is not None and x_intercept >= 0 and x_min <= x_intercept <= x_max:
                ...
                if len(new_x) >= 2:
                    ...
                    trace.update(line=dict(width=3, dash='solid', color='rgba(255, 0, 0, 0.8)'))
    ...
    if show_trendline and len(cell_type_df) >= 2:  # Need at least two points for correlation
        try:
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(
                cell_type_df['Age_for_plot'], 
                cell_type_df['Expression']
            )
            r_squared = r_value ** 2
    else:
