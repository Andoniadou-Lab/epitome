...
def plot_sc_dataset(adata, selected_gene, sort_order=False, color_map='viridis'):
    ...
    try:
        ...
        color_values = expression.toarray().flatten() if scipy.sparse.issparse(expression) else expression.flatten()
        ...
        for cell_type in cell_types:
            ...
            cell_type_fig.add_trace(go.Scatter(
                x=cell_coords[:, 0],
                y=cell_coords[:, 1],
                mode='markers',
                marker=dict(
...
def list_available_datasets(BASE_PATH,base_path, version="v_0.01"):
    ...
    try:
        ...
        datasets = [f for f in os.listdir(os.path.join(base_path,version,"epitome_h5_files")) if f.endswith('.h5ad')]
        ...
        for sra_id in sra_ids:
            dataset_info = curation_data[curation_data['SRA_ID'].str.contains(sra_id, na=False)]
