...
def create_figure(prop_df, cell_types, color_map, title, x_values=None, connect_bars=False):
    data = []
...
def create_proportion_plot(matrix, rows, columns, meta_data, selected_samples=None, selected_authors=None, 
                           only_normal=False, only_whole=False, group_by_sex=False, order_by_age=False, 
                           show_mean=False, use_log_age=False,atac_rna="rna"):
    ...
    if atac_rna == "rna":
        ...
        prop_df.columns = [meta_data[meta_data['GEO'] == sra_id]['SRA_ID'].values[0] for sra_id in prop_df.columns]
        valid_sra_ids = meta_data['SRA_ID'].unique()
    ...
    # Apply filters
    if selected_samples:
        valid_sra_ids = np.intersect1d(valid_sra_ids, meta_data[meta_data['Name'].isin(selected_samples)]['SRA_ID'].unique())
    if selected_authors:
        valid_sra_ids = np.intersect1d(valid_sra_ids, meta_data[meta_data['Author'].isin(selected_authors)]['SRA_ID'].unique())
    if only_normal:
        valid_sra_ids = np.intersect1d(valid_sra_ids, meta_data[meta_data['Normal'] == 1]['SRA_ID'].unique())
    if only_whole:
        valid_sra_ids = np.intersect1d(valid_sra_ids, meta_data[meta_data['Sorted'] == 0]['SRA_ID'].unique())
    ...
    if group_by_sex:
        ...
        male_data = prop_df[[id for id in prop_df.columns if id in male_subset['SRA_ID'].values]]
        female_data = prop_df[[id for id in prop_df.columns if id in female_subset['SRA_ID'].values]]
        ...
        if show_mean:
            ...
            if not male_data.empty:
                ...
                fig_male = create_stacked_area_plot(male_data, cell_types, color_map, male_ages)
            else:
                fig_male = None
            
            if not female_data.empty:
                female_data, female_ages = process_age_grouped_data(female_data, female_subset)
                fig_female = create_stacked_area_plot(female_data, cell_types, color_map, female_ages)
            else:
                fig_female = None
        ...
        fig_male = create_figure(male_data, cell_types, color_map, 'Cell Type Proportions - Male') if not male_data.empty else None
        fig_female = create_figure(female_data, cell_types, color_map, 'Cell Type Proportions - Female') if not female_data.empty else None
        return fig_male, fig_female, config, None
        ...
        title = 'Mean Cell Type Proportions' if show_mean else 'Cell Type Proportions by Sample'
