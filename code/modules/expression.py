...
def create_expression_plot(matrix, genes, meta_data, gene_name, additional_group=None, connect_dots=False, selected_cell_types=None):
    """
    ...
    if additional_group:
        ...
        plot_df['x_position'] = plot_df.apply(
            lambda row: position_map[(row['new_cell_type'], str(row[additional_group]))], 
            axis=1
        )
    ...
    if connect_dots:
        for sra_id in plot_df['SRA_ID'].unique():
            ...
            x_values = sra_df['x_position'] if 'x_position' in sra_df.columns else sra_df['new_cell_type']
    ...
    dynamic_width = max(900, min(2400, num_cell_types * base_width))  # Min 900px, Max 2400px
