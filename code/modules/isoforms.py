...
def create_isoform_plot(
    ...
    for idx, transcript in gene_transcripts.iterrows():
        ...
        expression = (
            isoform_matrix[idx].toarray().flatten()
            if scipy.sparse.issparse(isoform_matrix)
            else isoform_matrix[idx]
        )
    ...
    plot_df["Combined"] = pd.Categorical(
        plot_df["Combined"], categories=combined_cats, ordered=True
    )
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="log10(eCPM)", tickfont=dict(size=12)),
        showlegend=True,
        height=600,
        width=2000,
        margin=dict(b=100),
