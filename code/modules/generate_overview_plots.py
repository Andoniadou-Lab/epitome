...
def preprocess_data(df):
    ...
    df['Year'] = df['Year'].apply(lambda x: 'Unpublished' if 'unpublished' in x.lower() else x)
    
    # Clean up Sex column
    df['Sex'] = df['Sex'].apply(lambda x: 'mixed' if isinstance(x, str) and 'mixed' in x.lower() else x)
    df['Sex'] = df['Sex'].apply(lambda x: 'unclear' if isinstance(x, str) and '?' in x else x)
    
    # Add RNA/ATAC label for easier filtering
    df['rna_atac'] = df['sc_sn_atac'].apply(lambda x: 'ATAC' if x in ['atac', 'multi_atac'] else 'RNA')
    
    # Label multiome data
    df.loc[df['sc_sn_atac'].isin(['multi_atac', 'multi_rna']), '10X version'] = 'Multiome'
    
    # Convert Age_numeric to numeric
    df['Age_numeric'] = pd.to_numeric(df['Age_numeric'].astype(str).str.replace(',', '.'), errors='coerce')
...
def create_broken_axis_histogram(data, modality, n_bins=30, output_path=None):
    ...
    top_ylim = (max(max_height - 5, second_highest + 1), max_height + 1)  # Top plot: around the highest peak
    bottom_ylim = (0, min(second_highest + 5, max_height - 1))  # Bottom plot: up to the second peak + buffer
    ...
    sns.histplot(data=df_filtered, x='Age_numeric', kde=False, color='#4C72B0', ax=ax1, bins=n_bins)
    sns.histplot(data=df_filtered, x='Age_numeric', kde=False, color='#4C72B0', ax=ax2, bins=n_bins)
    ...
    ax1.set_yticklabels([int(y) if y.is_integer() else int(round(y)) for y in ax1.get_yticks()])
    ax2.set_yticklabels([int(y) if y.is_integer() else int(round(y)) for y in ax2.get_yticks()])
    ...
    fig.suptitle(f'Distribution of Age ({modality})', fontsize=8, fontweight='bold', y=0.98)
...
def create_cumulative_plots(df, base_path, version):
    ...
    df_rna['Year'] = pd.to_numeric(df_rna['Year'].replace('Unpublished', '2024'), errors='coerce')
    df_atac['Year'] = pd.to_numeric(df_atac['Year'].replace('Unpublished', '2024'), errors='coerce')
    ...
    combined_path = os.path.join(base_path, "data", 'figures', version, 'cumulative_ncell_over_years_combined.png')
    plt.savefig(combined_path, dpi=300, bbox_inches='tight')
    
    # Also save as SVG for better quality
    combined_svg_path = os.path.join(base_path, "data", 'figures', version, 'cumulative_ncell_over_years_combined.svg')
    ...
    print(f"RNA summary: {rna_total_papers} papers, {rna_total_samples} samples, {rna_total_cells:,} cells")
    print(f"ATAC summary: {atac_total_papers} papers, {atac_total_samples} samples, {atac_total_cells:,} cells")
...
def generate_overview_plots(base_path, version="v_0.01"):
    ...
    atac_output_path = os.path.join(figures_dir, 'age_distribution_histogram_small_atac.svg')
